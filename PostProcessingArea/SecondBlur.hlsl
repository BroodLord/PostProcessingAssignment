...
float4 main(PostProcessingInput input) : SV_Target
{
    ...
    for (int i = 1; i < 5; i++)
    {
        tc += SceneTexture.Sample(PointSample, input.sceneUV + float2(rt_w * offset[i], 0.0f)) * weight[i] +
        SceneTexture.Sample(PointSample, input.sceneUV - float2(rt_w * offset[i], 0.0f)) * weight[i];
    }
